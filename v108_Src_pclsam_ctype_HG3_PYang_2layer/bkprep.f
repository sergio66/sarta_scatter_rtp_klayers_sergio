...
!CALL PROTOCOL:
C    BKPREP( IPROF, ICLOUD, CTYPE, CFRAC, CPRTOP, CEMIS, LBOT, PSURF,
C       PLEV, PLAY, TEMP, LTOPC, CRHO, TTOPC, TLAC, FLAC )
...
C    type      name    purpose                     units
C    --------  ------  --------------------------  ---------------------
C    REAL      CEMIS   cloud emissivity            none?
...
C      =================================================================
       SUBROUTINE BKPREP( IPROF, ICLOUD, CTYPE, CFRAC, CPRTOP, CEMIS,
     $    LBOT, PSURF, PLEV, PLAY, TEMP, LTOPC, CRHO, TTOPC, TLAC, FLAC)
C      =================================================================
...
C      Input/Output parameters
       REAL  CEMIS           ! cloud emissivity

C      Output parameters:
       INTEGER  LTOPC        ! layer containing cloud top
       REAL   CRHO           ! reflectivity
...
C      Set cloud top emissivity and reflectivity
       IF (CEMIS .GE. 0.0 .AND. CEMIS .LT. 1.0) THEN
          IF (CTYPE .EQ. 0) THEN
C           black body
            CEMIS=1.0
            CRHO=0.0
...
C           no reflection
C           CEMIS is unchanged
            CRHO=0.0
          ELSEIF( CTYPE .EQ. 2) THEN
C           use CEMIS for rho but not emis
            CRHO=(1.0 - CEMIS)/PI
            CEMIS=1.0
          ELSE
