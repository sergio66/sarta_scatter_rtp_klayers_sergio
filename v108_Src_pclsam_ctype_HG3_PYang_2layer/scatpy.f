...
C      FOR PING YANGs CODE
       ...
       INTEGER IADD1OR2,IX1 
       REAL TEMPER_P1_ORIG(MAXLAY+1), GTAU_P1_ORIG(MAXLAY+1)
       REAL CLDLAYERFRAC1
       REAL TEMPER(MAXLAY+2), GTAU(MAXLAY+2,NANG_MAX)
C-----------------------------------------------------------------------
C      SAVE STATEMENTS
C-----------------------------------------------------------------------
C      none
...
C
      ...
      !!! since QWGT is really used only for backgnd thermal, which
      !!! is quite different than ours, put it at 3 (clear sky default)
      NANG = NANG_MAX-1
      NANG = 3   
      NANG = 1  
...
c bot cloud is C2
     ...
     $       (CFRAC2 .GT. 0)) THEN
       ...
       IF (CLDPRESS1 .LT. 0) THEN    !!! no cloud
         ...
         END IF
 10    CONTINUE
       IF (PLEV(NCLD1) .LT. CLDPRESS1) THEN
         NCLD1 = NCLD1 + 1
         GOTO 10
       ELSE
         GOTO 20
 ...
 20     CONTINUE
       ...
       IF (CLDPRESS2 .LT. 0) THEN  !!! no cloud
         ...
         END IF
 11    CONTINUE
       IF (PLEV(NCLD2) .LT. CLDPRESS2) THEN
         NCLD2 = NCLD2 + 1
         GOTO 11
       ELSE
         GOTO 21
 ...
 21      CONTINUE
       ...
       !! now find the upper cloud fraction in topmost layer
       CLDLAYERFRAC1=(PLEV(NCLD1)-CLDPRESS1)/(PLEV(NCLD1)-PLEV(NCLD1-1))
       CLDLAYERFRAC1 =  
     $    log(PLEV(NCLD1)/CLDPRESS1)/log(PLEV(NCLD1)/PLEV(NCLD1-1)) 
       IF (CLDLAYERFRAC1 .GE. 0.99) THEN 
         !!!cloud is at top of layer, no need to divvy up layer into 2 
         IADD1OR2 = 1     
         IX1      = 0    !!!artificial offset for NCLD 
       ELSE 
         IADD1OR2 = 2    !!!cloud is far from top of layer, so up layer into 2 
         IX1      = 1    !!!artificial offset for NCLD 
         END IF 
       print *,'scatpy NCLD1,CLDFRAC1,IADD1OR2 = ',NCLD1,CLDLAYERFRAC1, 
     $          IADD1OR2,PLEV(NCLD1),CLDPRESS1,PLEV(NCLD1-1) 
...
c       print *,'ABCDEF',0,CNGWA1,CPSIZ1,CLDPRESS1,NCLD1,IWINDEX1,' & ',
c     $         CNGWA2,CPSIZ2,CLDPRESS2,NCLD2,IWINDEX2,' & ',
c     $         IWINDEX12
       ...
       ELSE 
         ...
         DO L=NCLD1,LBOT+1 
           ...
           END DO 
         TEMPER(NCLD1) = TEMPER_P1_ORIG(NCLD1) +  
     $     CLDLAYERFRAC1*(TEMPER_P1_ORIG(NCLD1-1)-TEMPER_P1_ORIG(NCLD1)) 
         END IF 
 
       DO I=1,NCHAN

c        for layer to space transmittance; do view angle, then gaussian angles
c         DO L = 1,LBOT-1
c           GTAU(L,1) = TAU(L,I)
c           END DO
c         L = LBOT
c         GTAU(L,1) = TAU(L,I)*BLMULT
c         DO L = 1,LBOT       
c           DO N = 2, NANG
cc             GTAU(L,N) = GTAU(L,1) * cos(sva*pi/180)/raX(N)
c             GTAU(L,N) = GTAU(L,1) * 1/(secang(L)*raX(N))
c             END DO
c           END DO

         DO L = 1,LBOT-1
           GTAU_P1_ORIG(L) = TAU(L,I)
           END DO
         L = LBOT
         GTAU_P1_ORIG(L) = TAU(L,I)*BLMULT

       IF (IADD1OR2 .EQ. 1) THEN    !! really easy; stick to same OD array 
         DO L = 1,LBOT       
           GTAU(L,1) = GTAU_P1_ORIG(L)
           END DO
         DO L = 1,LBOT       
           DO N = 2, NANG
             GTAU(L,N) = GTAU(L,1) * 1/(secang(L)*raX(N))
             END DO
           END DO
        ELSE
          DO L=1,NCLD1-1 
             ...
             END DO 
           DO L=NCLD1,LBOT
             GTAU(L+1,1) = GTAU_P1_ORIG(L) 
             END DO 
           GTAU(NCLD1,1) = GTAU_P1_ORIG(NCLD1-1) +  
     $                  (1-CLDLAYERFRAC1)*TAU(NCLD1-1,I)           
           DO L = 1,LBOT+2
             DO N = 2, NANG
               GTAU(L,N) = GTAU(L,1) * 1/(secang(L)*raX(N))
               END DO
...
c              if (i .eq. 7) then
c                print *,'here 1',
c     $   i,cngwa1,cpsiz1,cldpress1,ncld1,iwindex1,' & ',
c     $     cngwa2,cpsiz2,cldpress2,ncld2,iwindex2,' & ',iwindex12
c                end if
              CALL ly2g(I,NLAY+IADD1OR2,TEMPER,GTAU,NANG,QWGT,
     &          C1V3,C2V,
     &          CNGWA2,CPSIZ2,NCLD2,0.0,CPSIZ1,NCLD1+IX1,IWINDEX2,
     &          FREQ(I),TSURF,EMIS(I),-1,raX,secang,
     &          SVA,SOLARTOA,SUNCOS,COSDAZ,
     &          RADC2)
            ENDIF
         ELSE
            RADC2=0.0
...
c                end if
              CALL ly2g(I,NLAY+IADD1OR2,TEMPER,GTAU,NANG,QWGT,
     &          C1V3,C2V,
     &          CNGWA2,CPSIZ2,NCLD2,CNGWA1,CPSIZ1,NCLD1+IX1,IWINDEX12,
     &          FREQ(I),TSURF,EMIS(I),-1,raX,secang,
     &          SVA,SOLARTOA,SUNCOS,COSDAZ,
     &          RADC12)
            ELSE
...
c                end if
     ...
     &            RADC1)
              ELSEIF (iwindex1 .EQ. 2) THEN
                ...
                CALL ly2g(I,NLAY+IADD1OR2,TEMPER,GTAU,NANG,QWGT,
     &            C1V3,C2V,
     &            0.0,CPSIZ2,NCLD2,CNGWA1,CPSIZ1,NCLD1+IX1,IWINDEX1,
     &            FREQ(I),TSURF,EMIS(I),-1,raX,secang,
     &            SVA,SOLARTOA,SUNCOS,COSDAZ,
     &            RADC1)
                END IF
