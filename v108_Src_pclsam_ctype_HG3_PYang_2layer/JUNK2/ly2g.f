...
c------------
c these ncld2,ncld4 variables get changed by the subroutine so set these dummy vars
c that can be changed
c------------
        ncld4 = ncld4IN
        ncld2 = ncld2IN
...
c-------------------------
cc why is he doing this here, and then again a few lines below???????
cc        DO i = 1, nlev
cc          DO j = 1, nang
cc            gtran( i, j ) = exp( -gtau( i, j ) )
cc          END DO
cc        END DO
...
c Initialize layer reflectances and transmittances
        ...
        DO i = 1, nlayer
          DO j = 1, nang
c            TRANSMI_C( i, j ) = gtran( i, j )
            TRANSMI_C( i, j ) = exp( -gtau( i, j ) )
          END DO
        END DO
c-------------------------------------------------------------------
...
c---------------------------------------
        ...
        DO i = 1, NLAYER
          ...
          CALL PLANCK( FF, (TEMP(i+1)+TEMP(i))/2, B_AVG )
          DO j = 1, nang
            IF ( i .eq. NCLD2 ) THEN
     ...
     &              B_TOP, B_BOT, B_AVG, B_UP( i, j ), B_DN( i, j ))
          END DO
        END DO
