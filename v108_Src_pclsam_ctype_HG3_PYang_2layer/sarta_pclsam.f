...
C    PY_FITTEDCOE : PING YANGS scattering coeffs
...
C 24 Mar 2008 S.Hannon       Add COSDAZ
C 25 Nov 2008 S.Hannon       Bug fix for cloud1 reflectivity (was crho2)
C 25 Jan 2009 S.Machado      Split into two codes, ones for normal PCLSAM
C                            other for Ping Yangs code for Water/Ice which
C                            Instead of g/m2, use tau500nm as input
...
C      FOR PING YANGs CODE
     ...
     $                   THET_idx,THET_idx_
       ...
       CHARACTER*128 FDIR, FJUNLI
       CHARACTER*256 FNCIRUS, FNWATER
       INTEGER IPYCODE     !-1 for ONE dust cloud and/or something else
                           !-1 for CLEAR SKY
                           !+1 for anything else (TWO black clouds, one ice,one water etc)
       INTEGER iOpenedPYDATA !!so we do not have to keep on opening the database
...
C***********************************************************************
       ...
       iOpenedPYDATA = -1  !! assume we do not need PING YANGS cirrus/water scat code
       print *,'Ice : de from 10-180 um, tauvis from 0.01 to 10'
       print *,'Liq : de from  2-100 um, tauvis from 0.05 to 110'
...
C      Check and prepare (top) cloud1
     ...
     $          LBOT, PSURF, PLEV, PLAY, TEMP, LCTOP1, CRHO1, TCTOP1,
     $          TEMPC1, CLRT1)
          ELSE
...
C      Check and prepare (bottom) cloud2
     ...
     $          LBOT, PSURF, PLEV, PLAY, TEMP, LCTOP2, CRHO2, TCTOP2,
     $          TEMPC2, CLRT2)
          ELSE
C            Determine which lookup table to use
...
c      determine the setting of IPYCODE
       IPYCODE = -1  !default to SARTA PCLSAM
       IF (CFRAC1 .EQ. 0 .AND. CFRAC2 .EQ. 0) THEN      
         IPYCODE = -1     !!! no clouds, clear sky
       ELSEIF (CFRAC1 .GT. 0 .AND. CFRAC2 .EQ. 0) THEN      
         IF (LBLAC1) THEN         
           IPYCODE = -1     !!! one black cloud
         ELSEIF (CTYPE1 .GE. 300)  THEN
           IPYCODE = -1     !!! one complex (dust) cloud
         ELSEIF (CTYPE1 .LT. 300)  THEN
           IPYCODE = +1     !!! one complex (water/ice) cloud, use PY
           END IF
       ELSEIF (CFRAC2 .GT. 0 .AND. CFRAC1 .EQ. 0) THEN      
         IF (LBLAC2) THEN         
           IPYCODE = -1     !!! one black cloud
         ELSEIF (CTYPE2 .GE. 300)  THEN
           IPYCODE = -1     !!! one complex (dust) cloud
         ELSEIF (CTYPE2 .LT. 300)  THEN
           IPYCODE = +1     !!! one complex (water/ice) cloud, use PY
           END IF
       ELSEIF (CFRAC1 .GT. 0 .AND. CFRAC2 .GT. 0) THEN      
         IF (LBLAC1 .AND. LBLAC2) THEN         
           IPYCODE = -1     !!! two black clouds
         ELSEIF (LBLAC2 .AND. (CTYPE1 .GE. 300))  THEN
           IPYCODE = -1     !!! bottom black, top complex (dust) cld
         ELSEIF (LBLAC2 .AND. (CTYPE1 .LT. 300))  THEN
           IPYCODE = +1     !!! bottom black, top complex (I/W) cld
         !! check this a little bit more
         ELSEIF (LBLAC1 .AND. (LBLAC2 .EQ. .FALSE.)) THEN
           IF (CTYPE1 .GE. 300) THEN
             IPYCODE = -1     !!! top black cloud, bottom D
           ELSEIF (CTYPE1 .LT. 300) THEN
             IPYCODE = +1     !!! top black cloud, bottom I/W
           END IF
         ELSEIF ((LBLAC1 .EQ. .FALSE.) .AND. (LBLAC2 .EQ. .FALSE.)) THEN
           IF ((CTYPE1 .GE. 300) .OR. (CTYPE2 .GE. 300)) THEN
             IPYCODE = -1     !!! top or bottom is D
           ELSE
             IPYCODE = +1     !!! top and bottom are I/W
             END IF
           END IF
         END IF
...
C      SET SOLARTOA = 0.0 (cannot handle it in PYs code for now)
C      ----------------------
       IF (IPYCODE .LT. 0) THEN
         !PCLSAM or CLEAR SKY
         CALL SCATUMBC(NCHAN,FREQ,TEMP,TAU,TAUZ,TAUZSN,EMIS,TSURF,
     $              SVA,SECANG,SUNFAC,HSUN,RHOSUN,RHOTHR,LABOVE,COEFF,
     $              DOSUN,LBOT,NLAY,FCLEAR,CFRA12,
     $              BLMULT,SUNCOS,COSDAZ,0.0,PLAY,
     ...
     $              IPYCODE,RAD)
       ...
       ELSE
         !PING YANG CODE
         IF (iOpenedPYDATA .LT. 0) THEN 
           FDIR    = '/home/sergio/SARTA_CLOUDY/PYANG_2layer/DATA/'
c          FJUNLI  = 'global_emis_slopeInterc_MYD11C3.A2003152_wvnum.bin.le'
           FJUNLI = 'FIRTM2_baum05.dat'
           CALL CAT12(FDIR,FJUNLI,FNCIRUS)
           FJUNLI = 'unf_water_c.dat'
           CALL CAT12(FDIR,FJUNLI,FNWATER)
           CALL load_rt_tables(FNCIRUS,FNWATER)
           iOpenedPYDATA = +1
           END IF
         CALL SCATPY(NCHAN,FREQ,TEMP,TAU,TAUZ,TAUZSN,EMIS,TSURF,
     $              SVA,SECANG,SUNFAC,HSUN,RHOSUN,RHOTHR,LABOVE,COEFF,
     $              DOSUN,LBOT,NLAY,FCLEAR,CFRA12,
     $              BLMULT,SUNCOS,COSDAZ,0.0,PLAY,
     $              CTYPE1,CNGWA1,CPSIZ1, CPRBO1,CPRTO1,
