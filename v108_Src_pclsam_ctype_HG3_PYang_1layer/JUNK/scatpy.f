...
C      FOR PING YANGs CODE
     ...
     &                  FitcoeRc, FitcoeTc, FitcoeRw, FitcoeTw
       ...
       REAL TEMPER(MAXLAY+1),TTRANS(MAXLAY+1), CLDPRESS
...
C
 ...
 20     CONTINUE
       ...
       print *,CLDPRESS,IWINDEX,NCLD,LBOT
...
C        Calculate bottom cloud2 radiance
     ...
     $         RHOTHR, LABOVE, COEFF, RADC2 )
            ELSE
              CALL CloudyIR_BT(TSURF,TEMPER,TTRANS,NCLD,NLAY+1,IWindex,
     &          FREQ(I),CPSIZ2,CNGWA2,SVA,EMIS(I),COSDAZ,SOLARTOA,
     &          R,T,RADC2,ISTAT)
c call calrad1
            ENDIF
         ELSE
...
C        Calculate combined cloud1+cloud2 radiance
         IF (CFRA12 .GT. 0.0) THEN
            IF (LBLAC2) THEN
              CALL CloudyIR_BT(TSURF,TEMPER,TTRANS,NCLD,NLAY+1,IWindex,
     &          FREQ(I),CPSIZ1,CNGWA1,SVA,EMIS(I),COSDAZ,SOLARTOA,
     &          R,T,RADC12,ISTAT)
c               CALL CALRAD1( 
