c to compile and run, do f77 -f -N3 read_fitted_database_py.f; a.out
c USE THIS as absoft did not like the unformatted statement when opening unit-23
c    /usr/bin/g77  -fcase-lower -ffixed-line-length-120 read_fitted_database_py.f; a.out
...
c      call read_fitted_database_py
c      end
...
c.....................................................................
       ...
       SUBROUTINE READ_fitted_database_PY       
...
c       USE COMMON_SPACE
     ...
     &                  FitcoeRc, FitcoeTc, FitcoeRw, FitcoeTw
      ...
      CHARACTER*120 caDir,caFile,caAll
      INTEGER iLen
 ...
 10   FORMAT(A120)
      DO i=1,120
        ...
        END DO
      caDir = 
     $  '/home/sergio/SARTA_CLOUDY/PYANG_1layer/cloudyRTM_01Dec2008/'
      iLen = 1
 20   CONTINUE
      IF (caDir(iLen:iLen) .NE. ' ') THEN
        iLen = iLen + 1
        GOTO 20
        END IF
      ...
      iDebug = -1
      IF (iDebug .GT. 0) THEN
        print *,'3 to <4 um  = 1 to ',iOffSet, ' = ',(iOffset-1)+1,
     $                           ' points'
        print *,'=4 to 20 um = ', iOffSet+1,' to 251 = ',
     $                           (251-(iOffSet+1)+1),' points'
        END IF
...
c For the Reflective function of cirrus clouds
      caFile = 'cloudcoeffs/fitedcoerc.dat'
      caAll = caDir(1:iLen-1) // caFile
      OPEN(3,FILE=caAll,STATUS='OLD')
      read(3,*)cc
      Do i=iOffSet,NWL
	read(3,*)Wave_length(i),cc
        IF (iDebug .GT. 0) print *,i,NWL,Wave_length(i)
     	do j=1,Ntcld
...
c For the albedo function of cirrus clouds
      caFile = 'cloudcoeffs/fitedcoetc.dat'
      caAll = caDir(1:iLen-1) // caFile
      OPEN(3,FILE=caAll,STATUS='OLD')
      read(3,*)cc
       Do i=iOffSet,NWL
...
c For the Reflective function of water clouds
      caFile = 'cloudcoeffs/fitedcoerw.dat'
      caAll = caDir(1:iLen-1) // caFile
      OPEN(3,FILE=caAll,STATUS='OLD')
      read(3,*)cc
       Do i=iOffSet,NWL
...
c For the Reflective function of water clouds
      caFile = 'cloudcoeffs/fitedcoetw.dat'
      caAll = caDir(1:iLen-1) // caFile
      OPEN(3,FILE=caAll,STATUS='OLD')
      read(3,*)cc
      Do i=iOffSet,NWL
