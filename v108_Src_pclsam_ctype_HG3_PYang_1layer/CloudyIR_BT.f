...
c     &      wvnum,De,Tauvis,obszenang,emis,
c     &      suncos,cosdaz, solarTOA, R,T, radtop,istat,i)
c +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
c     cosdaz : cos(solar-satellite) azimuth for solar HG
c   solarTOA : solarRAD at TOA

c  OUTPUT ...
c   TopBTcld:   brightness temperature (K) at top of atmosphere  >>>>> commented out
c          R:   cloud reflectivity
c          T:   cloud transmissivity
c          i : debug channel number
...
c     radwnplank:       Calculate the Planck radiance from wavenumber/temperature
...
c +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &      wvnum,De,Tauvis,obszenang,emis,suncos,cosdaz,solarTOA,
     &      R,T, radtop,istat)
	...
	real temper(*),ttrans(*)
	integer istat
        integer ncld,nlev,IWindex
        real tsurf,wvnum,De,Tauvis,obszenang,emis
        real R,T,radtop,suncos,solarTOA,cosdaz

	real wvx
	real plapro(mlev),taustar(mlev),tautstar(mlev)
	real wnplan
        real wvlen,rsurf,taus2,tauc2,temp,radsurf,radba,radbb,b1,b2,bbar
        real t1,t2,ts1,ts2,radbelow,btc,radcloud,radabove,radbc
        real tts1,tts2
        real wnbrit
        integer l
...
c ......... optical thickness of clouds is small enough to ignore
	   ...
	   call INTERPOLATION_RT(De,Tauvis,wvlen,obszenang,IWindex,R,T,
     $                           istat) 
c if we include the sun, remember scattering angle between 
c   X1 = (x1,phi1) and X2 = (x2,phi2) is
c cos(theta) = cos(x1)cos(x2) + sin(x1)sin(x2)cos(phi1-phi2)
	   if (istat .gt. 0) then
	     write (*,*) 'Interpolation_RT status=',istat
	   endif
	endif
	
c temper,ttrans,emis
c 	print *,tsurf,ncld,nlev,IWindex,
c     &      wvnum,De,Tauvis,obszenang,cosdaz,solarTOA,
c     &      R,T, radtop,istat
...
c * Define surface reflectivity
	rsurf=(1.0-emis)/3.1415927   !! sarta usually uses this!
c	rsurf=1.0-emis
