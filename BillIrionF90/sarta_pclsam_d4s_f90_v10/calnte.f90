...
!                               CO2TOP
! 19 Dec 2017 Bill Irion     Convert to F90
...
SUBROUTINE CALNTE ( INDCHN, TEMP, SUNCOS, SCOS1, VSEC1, &
	...
	REAL, INTENT(IN), DIMENSION(MAXLAY) :: TEMP
	REAL, INTENT(IN) :: SUNCOS ! solar zenith angle cosine at surface
	REAL, INTENT(IN) ::  SCOS1 ! solar zenith angle cosine at layer1
	REAL, INTENT(IN) ::  VSEC1 ! satellite view zenith angle secant at layer1
	INTEGER, INTENT(IN) :: NCHNTE
	INTEGER, INTENT(IN), DIMENSION(MXCNTE) :: CLISTN
	REAL, INTENT(IN), DIMENSION(NNCOEF,MXCNTE) :: COEFN
	REAL, INTENT(IN) :: CO2TOP ! CO2 mixing ratio in top layers (ppmv)
	!
	! Input/Output
	REAL, INTENT(INOUT), DIMENSION(MXCHAN) :: RAD
