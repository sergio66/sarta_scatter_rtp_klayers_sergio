...
SUBROUTINE MEAN_T(LBOT, PLEV, PSURF, TPSEUD, TLAY)
	...
	!
	! Input
	INTEGER, INTENT(IN) :: LBOT	! # of layers for this profile
	REAL, INTENT(IN) :: PSURF	! surface pressure
	REAL, INTENT(IN), DIMENSION(MAXLAY + 1) :: PLEV ! pressure levels
	REAL, INTENT(IN), DIMENSION(MAXLAY) :: TPSEUD   ! pseudo level temperature at PLEV(L+1)

	! Output
	REAL, INTENT(OUT), DIMENSION(MAXLAY) :: TLAY  ! layer mean temperature
