...
C 21 Feb 2007 Scott Hannon   Add IDCLD1, IDCLD2, CNGMIN, CFRMIN
C 23 Feb 2007 Scott Hannon   Remove VCLOUD (moved to FNMIE & re-sized)
...
c       PARAMETER( VSARTA = '1.07 07-03-02 PCLSAM')
       PARAMETER( VSARTA = '1.08 12-04-15 PCLSAM')
...
C      -----------------------------------
       ...
       PARAMETER(MAXPRO = 25)
       PARAMETER( MXGAS = 50) ! set to match MAXGAS in RTP "rtpdefs.f"
...
C      Cloud ID numbers (ie what to look for in HEAD.glist)
       ...
       INTEGER IDCLD2 ! cloud2 ID number
       PARAMETER( IDCLD1 = 201 )
       PARAMETER( IDCLD2 = 202 )
C
C      Cloud minimums
       REAL CNGMIN    ! min cngwat to consider (g/m^2)
       REAL CFRMIN    ! min cloud fraction to consider
       PARAMETER( CNGMIN = 1E-6 )
       PARAMETER( CFRMIN = 1E-4 )
C
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C Tuning filename
