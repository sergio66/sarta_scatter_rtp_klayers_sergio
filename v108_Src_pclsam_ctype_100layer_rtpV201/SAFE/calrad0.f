...
C    REALr     RHOSUN  solar surface reflectivity  none
...
C    21 Feb 2007 Scott Hannon   Remove LBOVE; change RHOSUN & RHOTHR
C                                  from array(MXCHAN) to scaler.
...
C      ------------------------
       IF (DOSUN) THEN
          RSUN=RHOSUN*SUNFAC*HSUN(I)*TAUZSN(I)
       ELSE
          RSUN=0.0
       ENDIF
C
...
C         Truncate F at limits as needed
       ...
       ENDIF
       RTHERM=RHOTHR*PI*RDOWN*F*TAUZ(I)
C
