...
C    AIRS
C
C    RDCOEF version with trace gases CO2, SO2, & HNO3 and two clouds
C
!F77====================================================================
...
C       MIETYP, MIENPS, MIEPS, MIEABS, MIEEXT, MIEASY )
...
C    30 Mar 2006 Scott Hannon   Add Mie variables
C    24 Apr 2006 Scott Hannon   Add call to FNMIE
C    05 Jan 2006 Scott Hannon   Fix MIEABS read loop: MIENPS not MXMIEA
...
C
       CLOSE(IOUN)
C
      write(6,*) 'TEST: RDCOEF after read set1'
C
C      ----------
C      Read set 2
C      ----------
...
C
       CLOSE(IOUN)
C
      write(6,*) 'TEST: RDCOEF after read set2'
C
C      ----------
C      Read set 3
C      ----------
...
C
       CLOSE(IOUN)
C
      write(6,*) 'TEST: RDCOEF after read set3'
C
C      ----------
C      Read set 4
C      ----------
...
C
       CLOSE(IOUN)
C
      write(6,*) 'TEST: RDCOEF after read set4'
C
C      ----------
C      Read set 5
C      ----------
...
C
       CLOSE(IOUN)
C
      write(6,*) 'TEST: RDCOEF after read set5'
C
C      ----------
C      Read set 6
C      ----------
...
C
       CLOSE(IOUN)
C
      write(6,*) 'TEST: RDCOEF after read set6'
C
C      ----------
C      Read set 7
C      ----------
...
C
       CLOSE(IOUN)
C
      write(6,*) 'TEST: RDCOEF after read set7'
C
C      ---------------------------
...
C
       CLOSE(IOUN)
C
      write(6,*) 'TEST: RDCOEF after read CO2 pert'
C
C      ---------------------------
...
C
       CLOSE(IOUN)
C
      write(6,*) 'TEST: RDCOEF after read SO2 pert'
C
C      ---------------------------
...
C
       CLOSE(IOUN)
C
      write(6,*) 'TEST: RDCOEF after read HNO3 pert'
...
C
       CLOSE(IOUN)
C
      write(6,*) 'TEST: RDCOEF after read N2O pert'
...
C
       CLOSE(IOUN)
C
      write(6,*) 'TEST: RDCOEF after read OPTRAN H2O'
C
C      -----------------------------------------------
...
C
       CLOSE(IOUN)
C
      write(6,*) 'TEST: RDCOEF after read thermal F'
C
C      -------
C      Read FX
C      -------
...
C
 ...
 1047     FORMAT('Error! Unexpected number of layers in fx file.',/,
     $    'Expected fx to have ',I4,' layers, but found ',I4)
       ENDIF
C
      write(6,*) 'TEST: RDCOEF after read fx'
...
C
      ...
      write(6,*) 'TEST: RDCOEF after read non-LTE'


CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C      Read Mie lookup tables

C      Assign particle type code numbers and filenames
C      note: unable to do this with include file, hence done with FNMIE
       CALL FNMIE( MIETYP, FNMIEA, FNMIEE, FNMIEG )
C
      write(6,*) 'TEST: RDCOEF after FNMIE, starting nmiety loop'

C      Loop over particle types
       DO K=1,NMIETY
C
      write(6,*) 'TEST: RDCOEF K=',K,' about to open mie abs file'
...
C
      ...
      write(6,*) 'TEST: RDCOEF K=',K,' about to read header'

C         Read the nchan and nmie
          READ(IOUN) I, IC
          IF (I .NE. MXCHAN) THEN
 ...
 1073        FORMAT('ERROR! invalid # of particle sizes in mie file',
     $       /,A80,/,'File has',I12,' sizes')
          ...
          MIENPS(K)=IC
C
      write(6,*) 'TEST: RDCOEF K=',K,' about to read sizes'

C         Read mie particle sizes
          READ(IOUN) (MIEPS(IL,K),IL=1,MIENPS(K))
C
      write(6,*) 'TEST: RDCOEF K=',K,' about to read mie abs table'
...
C
C
      write(6,*) 'TEST: RDCOEF K=',K,' about to open mie ext file'
...
C         Check particle sizes are consistent
 ...
 1075           FORMAT('ERROR! unexpected particle sizes in mie file',
     $             /,A80)
          ...
          ENDDO
C
      write(6,*) 'TEST: RDCOEF K=',K,' about to read mie ext table'

C         Read mie ext data for required channels
          J=1
          DO I=1,MXCHAN
C            Read data for this frequency/channel
             READ(IOUN) (MIEEXT(J,IL,K),IL=1,MIENPS(K))
C
C            Keep the data if the current channel is on the list
             IF (INDCHN(I) .NE. 0) THEN
                J=J + 1
...
C
C
      write(6,*) 'TEST: RDCOEF K=',K,' about to read mie asy'
C         ------------------------------
...
C         Read mie particle sizes
      ...
      write(6,*) 'TEST: RDCOEF xjunk=',(XJUNK(I),I=1,MIENPS(K))
...
C         Check particle sizes are consistent
          ...
          ENDDO
C
      write(6,*) 'TEST: RDCOEF K=',K,' about to read mie asy table'
...
c      write(6,*) 'TEST: RDCOEF about to read chan ID=',I

             READ(IOUN) (MIEASY(J,IL,K),IL=1,MIENPS(K))
C
C            Keep the data if the current channel is on the list
             IF (INDCHN(I) .NE. 0) THEN
                J=J + 1
      ...
      write(6,*) 'TEST: RDCOEF K=',K,' about to close asy file'
C
          CLOSE(IOUN)

      write(6,*) 'TEST: RDCOEF K=',K,' about to increment nmiety loop'
C
       ENDDO
C
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC

      write(6,*) 'TEST: RDCOEF near end'
