...
C    Apr 10 1995 Scott Hannon   New header comments; added ALT; new
C                               external function VACONV; SECANG may
C                               vary with layer
C    Jul  3 1995 Scott Hannon   Add parameter DZ/RDZ to RDPROF call
C     3 Feb 1997 Scott Hannon   Re-written for FWO+FOW+FMW+FCOW
...
C     3 May 2001 Scott Hannon   Add COMLEV; add PLEV to getbot call
C    13 Sep 2001 Scott Hannon   Changes to check of FCHAN vs FREQ
C     1 Nov 2002 Scott Hannon   Added SATZEN & SALT to RDRTP call, and
C                               if valid use SATZEN rather than SATANG
C     3 Jan 2003 Scott Hannon   Delete SUNSEC, add XZ & SUNFDG & code
...
C    22 Feb 2007 Scott Hannon   Extensive mods for 100 layer clouds
...
C      used locally only
       ...
       REAL VSTORE(4)      ! temporary storage for various variables
C
...
C      Assign AIRS satellite altitude (in kilometers)
       SALT=7.05E+2
...
C      ---------------------------
       CALL OPNRTP(FIN, PTYPE, NCHAN, FCHAN, LSTCHN, INDCHN,
...
C      ------------------------
     ...
     $ MIETYP, MIENPS, MIEPS, MIEABS, MIEEXT, MIEASY, VCLOUD)
C

C      ----------------------------
C      Update RTP attribute strings
C      ----------------------------
       CALL DOATTR(HATT, PATT, LRHOT, VCLOUD)
C
...
C         Warn and use default if invalid
          IF (SALT .LT. 655.0 .OR. SALT .GT. 755.0) THEN
             WRITE(IOINFO,1020) IPROF, SALT
 1020        FORMAT('Warning! Profile',I5,
     $          ': replacing invalid input satellite altitude ',
     $          1PE11.4,' with default 705 km')
             SALT=705.0
          ENDIF
       ELSE
          SALT=705.0
...
C
          DO L=1,LBOT
C            Sun angle used for transmittance calcs
             SZALAY=SACONV(RJUNK1,ALT(L))
             SECSUN(L)=SECANG(L) + 1.0E+0/COS(SZALAY)
C
C            Actual sun angle
             SZALAY=SACONV(SUNANG,ALT(L))
             SCOSL(L)=COS(SZALAY)
             SSECL(L)=1.0/SCOSL(L)
          ENDDO
...
C      Get basic cloud info from input RTP
       CALL GETCLD( IPROF, ICLD1, ICLD2, HEAD, PROF,
...
C            Prepare selected lookup table for given cpsize
             Call CCPREP( NCHAN, LBOT, INDMI1, MIENPS, CNGWA1, CPSIZ1, 
     $          MIEPS, MIEABS, MIEEXT, MIEASY,
     $          LBOTC1, LTOPC1, G_ASY1, NEXTB1, NSCAB1 )
          ENDIF
       ENDIF
...
C            Prepare selected lookup table for given cpsize
             Call CCPREP( NCHAN, LBOT, INDMI2, MIENPS, CNGWA2, CPSIZ2, 
     $          MIEPS, MIEABS, MIEEXT, MIEASY,
     $          LBOTC2, LTOPC2, G_ASY2, NEXTB2, NSCAB2 )
          ENDIF
       ENDIF

ccc uncomment for testing
c      if (IPROF .EQ. 1) then
c        print *, 'ctype1,ltopc1,tbotc1=',CTYPE1,LTOPC1,LBOTC1
c        print *, 'ctype2,ltopc2,tbotc2=',CTYPE2,LTOPC2,LBOTC2
c        do I=1,LBOT
c          print *,'layer, cngwa1, cngwa2=',I,CNGWA1(I),CNGWA2(I)
c        enddo
c      endif
ccc
...
C            Calc black1 + complex2
     ...
     $             NEXTB2, NSCAB2, G_ASY2, LTOPC2, LBOTC2, RADC12 )
             ENDIF
C
...
C         Calc complex cloud1
     ...
     $          CNGWA1, NEXTB1, NSCAB1, G_ASY1, LTOPC1, LBOTC1, RADC1 )
          ENDIF
C
...
C            Calc complex1+black2
     ...
     $             NEXTB1, NSCAB1, G_ASY1, LTOPC1, LBOTC1, RADC12 )
             ENDIF
C
C            Restore original values
...
C         Calc complex cloud2
     ...
     $          COSANG, SECANG, SCOSL, SSECL, TAU, TRANL, TRANZ,
     $          SUNFAC, HSUN, TRANS, RHOSUN(I), RHOTHR(I), COEFF,
     $          CNGWA2, NEXTB2, NSCAB2, G_ASY2, LTOPC2, LBOTC2, RADC2 )
          ENDIF
C
C         Calc complex cloud1 + complex cloud2
     ...
     $          CNGWA2, NEXTB2, NSCAB2, G_ASY2, LTOP12, LBOT12, RADC12)
          ENDIF
C
...
C      but clouds are generally below the altitude where nonLTE occurs.
       IF (DOSUN) THEN
          CALL CALNTE ( INDCHN, TEMP, SUNCOS, SCOSL(1), SECANG(1),
