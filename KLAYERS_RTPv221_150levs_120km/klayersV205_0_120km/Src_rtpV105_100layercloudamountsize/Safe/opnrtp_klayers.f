...
C 22 Aug 2007 Scott Hannon      Add IF-THEN to CHKGUC call; update
C                                  HEAD.gunit for GUCPS
...
ccc
       ...
       IF (HEAD.mrho    .EQ. BAD) HEAD.mrho=0
       IF (HEAD.memis   .EQ. BAD) HEAD.memis=0
       IF (HEAD.mwmemis .EQ. BAD) HEAD.mwmemis=0
       IF (HEAD.mwmstb  .EQ. BAD) HEAD.mwmstb=0
...
C               Note: do not check cloud particle size units
                IF (GLISTF(NGASF) .LT. IDCLD1+IOFFPS) THEN
...
C      Update ngas & glist & gunit
       ...
       DO I=1,NWANT
          ...
          HEAD.gunit(I)=GUCOUT
C         Adjust GUCOUT for cloud amount and/or particle size as needed
          IF (LISTG(I) .GE. IDCLD1) THEN
             IF (LISTG(I) .LT. IDCLD1+IOFFPS) THEN
                HEAD.gunit(I)=GUCCLD
             ELSE
                HEAD.gunit(I)=GUCPS
             ENDIF
