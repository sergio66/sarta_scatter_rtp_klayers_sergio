...
C 06 Sep 2007 Scott Hannon      Revise AMULT for particle size
...
C      Loop over the layers
       DO IL=1,NLAY
          ...
          PROF.plays(IL)=PLAY(ILR)*1013.25  ! convert atm to mb
...
C         Note: this is the unit used by AIRS level2 processing
          DO IG=1,NGASES
             IF (LISTG(IG) .GE. IDCLD1) THEN
                IF (LISTG(IG) .GE. IDCLD1+IOFFPS) THEN
                   AMULT=1.0
                ELSE
...
c     $       PROF.plays(IL), PROF.ptemp(IL),
c     $       (PROF.gamnt(IL,IG),IG=1,ILR)
c 2010     FORMAT(I4,10(1X,1PE11.4))
c       ENDDO
ccccccccccccccc
C
       RETURN
