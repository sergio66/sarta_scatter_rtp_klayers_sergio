...
C 17 Dec 2004 Scott Hannon - Add PTYPE to RDRTP call; add NLAY to GETBOT
C                               call.
C 19 Aug 2005 Scott Hannon - MODIS version; new RDCOEF & CALPAR vars
C 22 Aug 2005 Scott Hannon - test version with FMEAN
...
C      for RDCOEF             ! Info for selected channels only
       ...
       REAL COEFNU(NCOFNU,MAXLAY,MXCHAN) ! chan freq coefs
C
C      for rtpopen
       INTEGER rtpopen
       CHARACTER*1 MODE
...
C      for RDRTP; profile to calculate
       ...
       REAL  FAMNT(MAXLAY) ! prof layer "fixed" (CO2) amount
...
C      for CALPAR
       ...
       REAL CO2PRD(  NCO2,MAXLAY) ! CO2 pert perdictors
C
C      for CO2 perturbation
       REAL CO2MLT                ! CO2 perturbation multiplier
C
C      for CALOWP
       REAL  WAANG(MAXLAY)
       INTEGER LOPMIN
...
c for fchan_data
       ...
       INTEGER IOJUNK
       INTEGER IP
       REAL  FMEAN(MAXLAY,MXCHAN),FJUNK(MAXLAY)
c
c for ctrans_data
       INTEGER IOJNK2
       REAL  CTRANS(MAXLAY,MXCHAN)
ccc
...
ccc
c open the fchan_data file
       IOJUNK=IOUN + 30
       OPEN(UNIT=IOJUNK, FILE='fmean_data.dat', STATUS='OLD',
     $     FORM='UNFORMATTED', IOSTAT=IERR)
       IF (IOERR .NE. 0) THEN
          WRITE(6,*) IERR, ': error opening fchan_data.dat'
          STOP
       ENDIF
ccc
c open the ctrans_data file
       IOJNK2=IOUN + 31
       OPEN(UNIT=IOJNK2, FILE='ctrans_data.dat', STATUS='OLD',
     $     FORM='UNFORMATTED', IOSTAT=IERR)
       IF (IOERR .NE. 0) THEN
          WRITE(6,*) IERR, ': error opening ctrans_data.dat'
          STOP
       ENDIF
ccc
...
C      -----------------------------
     ...
     $    RFAMNT, RWAMNT, ROAMNT, RCAMNT, RMAMNT)
C
...
C      ---------------------------
     ...
     $    IH2O, IO3, ICO, ICH4, IOPCI, HEAD, HATT, PATT)
C
...
C      ------------------------
     ...
     $   WAVGOP, COFH2O, FX, COEFNU )
C
C      Get and apply multipler tuning to coefficients
...
C      Read input RTP
C      --------------
       CALL RDRTP( LWANT, IPROF, IOPCI, IH2O, IO3, ICO, ICH4,PTYPE,RALT,
     $    NLAY, NEMIS, NRHO, LAT, LON, SATANG, SATZEN, SALT, SUNANG, 
     $    PSURF, TSURF, CO2PPM, XEMIS, FEMIS, XRHO, FRHO,
...
C         assuming T is in linear in log(P)
     ...
     $       LOG( PLAY(LBOT)/PLAY(LBOT-1) )             ! slope
       ...
       ENDIF
C
C      Assign CO2 perturbation
       CO2MLT=100*( CO2PPM - CO2STD )/(3.0*CO2STD)
C      Note: CO2 perturbation coefs are normalized for +3% CO2
       IF (ABS(CO2MLT) .LT. 0.05) CO2MLT=0.0
C
C      Check satellite elevation
       IF (SALT .GT. 0) THEN
C         Warn and use default if invalid
          IF (SALT .LT. 655 .OR. SALT .GT. 755) THEN
...
ccc
c read fchan_data for this profile
      DO ICLOOP=1,MXCHAN
         READ(IOJUNK) IC,IP,(FJUNK(L),L=1,MAXLAY)
         IF (IPROF .NE. IP) WRITE(6,*) 'fchan_data IPROF,IP',IPROF,IP
         IF (INDCHN(IC) .NE. 0) THEN
            DO L=1,MAXLAY
               FMEAN(L,INDCHN(IC))=FJUNK(L)
            ENDDO
         ENDIF
      ENDDO
ccc
c read ctrans_data for this profile
      DO ICLOOP=1,MXCHAN
         READ(IOJNK2) IC,IP,(FJUNK(L),L=1,MAXLAY)
         IF (IPROF .NE. IP) WRITE(6,*) 'ctrans_data IPROF,IP',IPROF,IP
         IF (INDCHN(IC) .NE. 0) THEN
            DO L=1,MAXLAY
               CTRANS(L,INDCHN(IC))=FJUNK(L)
            ENDDO
         ENDIF
      ENDDO
ccc
...
C
       CALL CALPAR (LBOT,
     $     RTEMP, RFAMNT, RWAMNT, ROAMNT, RCAMNT, RMAMNT,
     $      TEMP,  FAMNT,  WAMNT,  OAMNT,  CAMNT,  MAMNT,
...
C
     ...
     $    FIXMUL, CONPRD, FPRED1, WPRED1, OPRED1,
...
C
     ...
     $    FIXMUL, CONPRD, FPRED2, OPRED2, WPRED2, CO2PRD,
     $    INDCO2, COFCO2, CO2MLT, TAU, TAUZ)
C
       CALL CALT3( INDCHN, LBOT, BLMULT, NCHN3, CLIST3, COEF3,
     $    FIXMUL, CONPRD, FPRED3, MPRED3, WPRED3,
     $    INDH2O, H2OPRD, COFH2O, LOPMIN, LOPMAX, LOPLOW,
     $    LOPUSE, WAOP, DAOP, WAANG, TAU, TAUZ)
C
       LTAU=.TRUE.
...
C
     ...
     $    CO2PRD, INDCO2, COFCO2, CO2MLT, XZ, TAU, TAUZ )
C
...
C
          CALL CALT4( LTAU, INDCHN, LBOT, BLMULT, NCHN4, CLIST4,
     $       COEF4, FIXMUL, CONPRD, FPRED4, CPRED4, OPRED4, WPRED4,
     $       CO2PRD, INDCO2, COFCO2, CO2MLT, XZ, TAU, TAUZSN )
C
          CALL CALT5( LTAU, INDCHN, LBOT, BLMULT, NCHN5, CLIST5,
     $       COEF5, FIXMUL, CONPRD, FPRED5, WPRED5, OPRED5,
     $       CO2PRD, INDCO2, COFCO2, CO2MLT, XZ, TAU, TAUZSN )
...
C      ----------------------
       CALL CALRAD ( NCHAN, FREQ, TAU, TEMP, RTEMP, WAMNT, RWAMNT,
     $    TSURF, EMIS, LBOT, SUNCOS, RHOSUN, DISTES, HSUN, TAUZSN,
