...
C      version template    '#.## YY-MM-DD <--------comment--------->'
       PARAMETER( VSARTA = '1.05 05-08-15 w/MODIS chan freq shift' )
       PARAMETER( VSCOEF = '05-08-15 SRF=mean Aqua tun=AIRSjan04')
...
C      ----------------------
       INTEGER NCOFNU ! number of channel freq coefs (5)
       PARAMETER(NCOFNU = 5)
...
C
     ...
     $ '/asl/data/sarta_database/Data_MODIS/Solar/solar_data.txt')
       PARAMETER(FNNU =
     $ '/asl/data/sarta_database/Data_MODIS/Coef/fchan_pad.dat')
