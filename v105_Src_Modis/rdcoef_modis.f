...
C    REAL arr  COEFNU  channel freq (nu) coefs     various
...
C    12 Aug 2005 Scott Hannon   Add COEFNU for MODIS
...
C
ccc for testing
c       write(6,*) 'ICHAN, FRQCHN, LACHAN, FCHAN',ICHAN,FRQCHN,LACHAN,
c     $ (FCHAN(IC),IC=1,NFCOEF)
ccc
C         Keep the data if the current channel is on the list
          IF (INDCHN(ICHAN) .NE. 0) THEN
...
C
C      -----------------------
C      Read channel freq coefs
C      -----------------------
       OPEN(UNIT=IOUN,FILE=FNNU,FORM='UNFORMATTED',STATUS='OLD',
     $    IOSTAT=IERR)
       IF (IERR .NE. 0) THEN
          WRITE(6,1020) IERR, FNNU
          STOP
       ENDIF
C
       J=1
       DO I=1,MXCHAN
C         Read data for this frequency/channel
          READ(IOUN) ICHAN, NU0, ((CJUNK(IC,IL),IC=1,NCOFNU),
     $       IL=1,MAXLAY+1)
C
C         Keep the data if the current channel is on the list
          IF (INDCHN(ICHAN) .NE. 0) THEN
             FREQ( INDCHN(ICHAN) )=NU0
             DO IL=1,MAXLAY+1
                DO IC=1,NCOFNU
                   COEFNU(IC,IL, INDCHN(ICHAN) )=CJUNK(IC,IL)
                ENDDO
             ENDDO
          ENDIF
       ENDDO
