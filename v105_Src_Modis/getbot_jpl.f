...
C    AIRS
C
C    GETBOT_jpl : this version uses JPL's delpx=5 mb
C
!F77====================================================================
...
C    09 Jun 2005 Scott Hannon   "jpl" version (ie DELPX=5)
...
C      Determine LBOT from PSURF by comparing to PLEV
       LBOTX=MAXLAY
c       DELPX=0.05*( PLEV(LBOTX+1) - PLEV(LBOTX) )
       DELPX=5.0 ! jpl version uses 5 mb
 10    IF (PSURF .LT. PLEV(LBOTX)+DELPX) THEN
          LBOTX=LBOTX - 1
          GOTO 10
       ENDIF
C
