...
C    22 Aug 2005 Scott Hannon   test version with FMEAN & CTRANS
...
C-----------------------------------------------------------------------
       ...
       REAL COEFNU(NCOFNU,MAXLAY,MXCHAN)
       REAL   TAUZ(MXCHAN)
       REAL    RAD(MXCHAN)
       REAL     BT(MXCHAN)
       REAL  FMEAN(MAXLAY,MXCHAN)
...
c test version using input variable CTRANS
      TZ = CTRANS(L,I)
      TAUZ(I) = CTRANS(LBOT,I)
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
...
ccc
c             FSHIFT = COEFNU(1,L,I) +
c     $                COEFNU(2,L,I)*TP(L) +
c     $                COEFNU(3,L,I)*TZ +
c     $                COEFNU(4,L,I)*(TZM1 - TZ)/SECANG(L) +
c     $                COEFNU(5,L,I)*TZ*TZ
ccc
C
             FPLNCK = FREQ(I) + FSHIFT

ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c test version using input variable FMEAN
      FPLNCK = FMEAN(L,I)
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
...
c      FPLNCK=FREQ(I)
ccc
c      IF (ABS(FSHIFT) .GT. 100) THEN
c         write(6,*) 'I,L,FCHAN0,FSHIFT:',I,L,FREQ(I),FSHIFT
c         write(6,*) 'COEFNU    :',(COEFNU(J,L,I),J=1,NCOFNU)
c         write(6,*) 'predictors:',(NUPRED(J,L),J=1,NCOFNU)
c      ENDIF
ccc
...
ccc for testing
c      RJUNK = (TZM1 - TZ)*RPLNCK(L)
c      write(6,*) 'RPLNCK,TZM1,TZ,RJUNK:',RPLNCK(L),TZM1,TZ,RJUNK
ccc

          ENDDO
C
C         Add on the bottom surface emission
c removed for testing          RAD(I)=RAD(I) + TAUZ(I)*EBOT(I)*C1V3/( EXP( C2V/TBOT ) - 1.0 )
