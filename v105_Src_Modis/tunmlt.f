...
!DESCRIPTION:
C    February 2003 version of SARTA v1.04 code by L.Strow/S.Hannon.
C
...
C       1    2   3    4     5    6   7   8    9
C       ID RJUNK XF XH2OL XH2OC XO3 XCO XCH4 XCO2
C    where
C       ID = integer, channel ID number
C       RJUNK = real, value is ignored, eg perhaps channel freq
C       XF,XH2OL,XH2OC,XO3,XCO,XCH4,XCO2 = real, tuning multipler
C          for fixed, water lines, water continuum, ozone, carbon
C          monoxide, methane, and carbon dioxide.  The value should
...
ccc
c       write(6,*) 'usemlt(fixed,H2Oline,H2Ocon,O3,CO,CH4,CO2p)=',
c     $    (USEMLT(J),J=1,7)
ccc
       ...
       IF (USEMLT(7)) THEN
          DO ICHAN=1,MXCHAN
             IF (INDCO2(ICHAN) .GT. 0) THEN
                ...
                DO L=1,MAXLAY
                   DO J=1,NCO2
                      COFCO2(J,L,I)=XMULTS(2,ICHAN)*COFCO2(J,L,I)
                   ENDDO
                ENDDO
             ENDIF
          ENDDO
