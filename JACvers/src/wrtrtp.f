...
C    INTEGER   NWANTC  if > 0, this is user specified none
C    INT arr   RINDCHN  index of chans being used, esp if NWANTC > 0
...
C      =================================================================
       SUBROUTINE WRTRTP(IP, IOPCO, NCHAN, RAD, PROF, NWANTC, RINDCHN)
C      =================================================================
...
C      Input parameters:
       ...
       INTEGER  IOPCO
       INTEGER  NCHAN
       REAL    RAD(MXCHAN), ROBS1(MXCHAN)
       INTEGER NWANTC
       INTEGER RINDCHN(MXCHAN) ! list of selected channels
C
...
C           Convert from Watts/m^2/cm^-1 to milliWatts/m^2/cm^-1
       ...
       ELSEIF (NWANTC .GT. 0) THEN
         PROF%rcalc = 0
         ROBS1(1:size(ROBS1)) = PROF%ROBS1(1:size(ROBS1))
         DO I=1,NCHAN
C           Convert SARTA cals from Watts/m^2/cm^-1 to milliWatts/m^2/cm^-1
            PROF%rcalc(I)=RAD(I)*1000.0
C           KEEP ROBS1 in same units
            PROF%robs1(I)=ROBS1(RINDCHN(I))
         ENDDO
       END IF
C
C      -------------------------
