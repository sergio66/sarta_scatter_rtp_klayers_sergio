...
C    INTEGER   RINDCHN  list of channel indices    none (1-2378)
...
C      =================================================================
     ...
     $    ICO2, ISO2, IHNO3, IN2O, INH3, IOPCI, HEAD, HATT, PATT, 
     $    LCO2PM, NWANTC, LISTC, RINDCHN)
C      =================================================================
...
C      Output
       ...
       INTEGER RINDCHN(MXCHAN) ! indices of channels for robs1
...
c       INTEGER     IG
       ...
       INTEGER      J
       INTEGER      K
       INTEGER      IK,INTERSECT,IACHAN(MXCHAN),IACHANNEW(MXCHAN)
...
C        
 ...
 1044          FORMAT('ERROR! input RTP has repeat of ',
     $         'channel ',I4)
       ...
       ELSEIF (NWANTC .GT. 0) THEN
         K=0  ! initialize counter
         IACHAN = 0
         IACHANNEW = 0
         IACHAN(1:HEAD%NCHAN) = HEAD%ICHAN(1:HEAD%NCHAN)
         NCHAN = NWANTC
         DO I=1,NCHAN
            J=LISTC(I)              ! channel ID
            IK = INTERSECT(J,IACHAN,HEAD%NCHAN)
            IF (IK .LT. 0) THEN
              print *,'ERROR : you entered a channel list on command line, 
     $ but cannot find channel in rtp h.ichan list ',J
              STOP
            END IF
            FCHAN(I)=HEAD%vchan(IK)  ! channel freq (or junk if unfilled)
            IACHANNEW(I)=iachan(IK)
            RINDCHN(I) = IK
!            print *,I,J,IK,IACHANNEW(I),FCHAN(I)
!            stop

            IF ((J .LT. 1) .OR. (J .GT. MXCHAN)) THEN
               WRITE(IOERR,1046) MXCHAN, J
 1046          FORMAT('Error! Channel number is out of range.',/,
     $         'Range is 1 to ',I4,', but input RTP has ',I7)
               STOP
            ENDIF       
C        
...
C              Not a repeat
               ...
               LSTCHN(K)=J
               INDCHN(J)=K
C               write(*,'(A,4(I5),F12.5,3(I5))') 'moaiw',I,J,K,IACHANNEW(I),FCHAN(I),LSTCHN(K),INDCHN(J),RINDCHN(I)
C        
            ELSE
               WRITE(IOERR,1048) J
 1048          FORMAT('ERROR! input RTP has repeat of ',
     $         'channel ',I4)
               STOP
            ENDIF
         ENDDO
...
!       print *,'miaow in opnrtp_pclsam.f'
!       print *,NCHAN
!       print *,FCHAN(1:NCHAN)
!       print *,LSTCHN(1:NCHAN)
!       print *,INDCHN(LSTCHN(1:NCHAN))
!!       print *,INDCHN(LSTCHN(1:NCHAN))
!       print *,INDCHN((1:NCHAN))
!       stop
!       print *,'miaow out opnrtp_pclsam.f'
