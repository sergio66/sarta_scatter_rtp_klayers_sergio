...
\item   jacunit = 0,1,2
  \begin{itemize}  
  \item 0 : units are d(rad)/dX (ie d(rad)/dT or d(rad)/dq)
  \item 1 : units are d(BT)/dX (ie d(BT)/dT or d(BT)/dq)
  \item 2 : units are q(dBT)/dX (ie d(BT)/dT or d(BT)/dq) <DEFAULT>
  \end{itemize}  
...
\]
which for numerical integration reduces to (see eg \citep{mac:17*2})
\[
