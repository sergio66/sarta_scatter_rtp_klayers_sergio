...
figure(9); clf; 
  figure(9); hold on; plot(h.vchan,10*sum(jacx.tjac(:,1:97)'),'r.-',h.vchan,10*sum(dd(:,1:97)'),'m'); hold on
figure(10); clf; 
  figure(10); hold on; plot(h.vchan,sum(jacx.tjac(:,1:97)')./sum(dd(:,1:97)'),'b'); hold on; axis([640 1640 0 +2]); 
...
figure(10); hold on; plot(h.vchan,sum(jacx.wvjac(:,1:97)')./sum(dd(:,1:97)'),'r'); hold on; axis([640 1640 0 +2]); 
...
figure(10); hold on; plot(h.vchan,sum(jacx.o3jac(:,1:97)')./sum(dd(:,1:97)'),'k'); hold on; axis([640 1640 0 +3]); title('ratio FINITE/ANALYTIC'); hl = legend('TZ','WV','O3','location','best','fontsize',10);
