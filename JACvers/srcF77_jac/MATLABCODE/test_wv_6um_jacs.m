...
% see lines 459 and 699 in ycalt2_od.f
 time ../bin/jac_airs_l1c_2834_cloudy_may19_prod_debug fin=newdayx_nocldfields.op.rtp  fout=newdayx_1_100_12150.rad.rtp                    listp=1  listj=1  listc=1603 >& ugh0_water
 time ../bin/jac_airs_l1c_2834_cloudy_may19_prod_debug fin=newdayx_nocldfields_pertWVx1.1.op.rtp  fout=newdayx_1_100_12150.rad.rtp         listp=1  listj=1  listc=1603 > ughpert_water
%}
...
junk = load('ugh0_water');    len = length(junk); rawrad  = junk(len,3); junk = junk(1:len,3); rad_ODjac  = reshape(junk,nlays,2); %% OZ only, but has correct jas in last 100
junk = load('ughpert_water'); len = length(junk); pertrad = junk(len,3); junk = junk(1:len,3); pert_ODjac = reshape(junk,nlays,2); %% OZ only, but has correct jac in last 100

figure(1); clf;  semilogx(rad_ODjac(:,1),1:nlays,pert_ODjac(:,1),1:nlays); title('ODs raw(b) and pert (r)'); set(gca,'ydir','reverse');
...
figure(3); clf; semilogx(rad_ODjac(:,1),1:nlays,rad_ODjac(:,1)+rad_ODjac(:,2).*dx,1:nlays,'r.-',rad_ODjac(:,1)+jac_here.*dx,1:nlays,'kx-'); set(gca,'ydir','reverse'); 
  hl = legend('correct new OD','new OD from SARTA analytic jac','new OD from jac\_here','location','best','fontsize',10);

%junk = load('ugh0_water');    len = length(junk); rawrad  = junk(len,4); junk = junk(1:len,3); rad_ODjac  = reshape(junk,nlays,2);  %% OZ + other gases is more correct, but has 0 in last 100
%junk = load('ughpert_water'); len = length(junk); pertrad = junk(len,4); junk = junk(1:len,3); pert_ODjac = reshape(junk,nlays,2);  %% OZ + other gases is more correct, but has 0 in last 100
