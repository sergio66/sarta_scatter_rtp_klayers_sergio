...
c-----------------------------------------------------------------------
      ...
      SUBROUTINE ly2g( nlev, temp, gtau, nang, qwgt, 
     $                 tauvis4, de4, ncld4, tauvis2, de2, ncld2,
     $                 cld_indt, waven, tsfc, emiss, sfc_type, 
...
c------------
        ...
        REAL tauvis2,de2,tauvis4,de4,tsfc
        REAL solartoa,solarcos,dazi
        INTEGER ncld2,ncld4,cld_indt,waven,tsdc,emiss,zen,rad
...
c %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c 'PROD' is a fortran function to calculate the total transmittance for given layers if those 
c layers are above an given level.  If the given level in the input is lower than the layer 
c transmittance, the function simply returns 1.

c index is the angle index
      REAL FUNCTION prod(raaTau,iAngleIndex,nlayer,istart,istop)

        IMPLICIT NONE
        INCLUDE 'pyang2cld.param'

        REAL raaTau( NLEVEL_MAX - 1, NANG_MAX )
        INTEGER iAngleIndex

        INTEGER nlayer,istart,istop

        REAL rx
        INTEGER i
        
        rx = 1.0
        IF (iSTOP .GT. nlayer) THEN
          print *,'error in real fcn prod : too many layers'
          STOP
          END IF

        IF (istart .le. istop) then
          DO i = istart,istop
            rx = rx * raaTau(i,iAngleIndex)
            END DO
          END IF
