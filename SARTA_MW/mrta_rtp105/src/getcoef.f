...
C   LOCAL VARIABLES
      ...
      INTEGER NFREQ,NLEV,NCOEF,NDESCR,L,NCH,J,I
      LOGICAL START/.TRUE./
      SAVE START
C
      IF(START) NCHREAD = 0
...
5      FORMAT(A6)
      END DO
C
      IF(NCHREAD.EQ.0) THEN ! READ STANDARD PROFILE FOR TRANSMITTANCE
       NTRLEV = NLEV
       DO L=1,NLEV
        READ(LUNTR,1,IOSTAT=IOS,ERR=90) PSTD(L),TSTD(L),PLB(L)
1       FORMAT(2F10.3,30X,F10.3)
       END DO
      ELSE !SKIP STANDARD PROFILE, BECAUSE IT'S ALREADY IN COMMON
       DO L=1,NLEV
        READ(LUNTR,5,IOSTAT=IOS,ERR=90) DESCR
       END DO
      ENDIF
