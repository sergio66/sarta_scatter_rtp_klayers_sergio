...
c
       ...
       if (iInstr .EQ. 1) THEN
         ...
         print *, 'AMSU-A coeff file status =', iret
         close(iounit)

       elseif (iInstr .EQ. 2) THEN
         open(iounit,file=tramsub,status='old',form='formatted')
         call getcoef(iounit, iret, freq )
         print *, 'AMSU-B coeff file status =', iret
         close(iounit)

       elseif (iInstr .EQ. 3) THEN
         open(iounit,file=tratms,status='old',form='formatted')
         call getcoef(iounit, iret, freq )
         print *, 'ATMS coeff file status =', iret
         close(iounit)
...
c	  print *,'mwcalc',n,td,tr,e,er,numlev
c	  print *,'     t',(t(kindex),kindex=1,numlev)
c	  print *,'  tran',(tran(kindex),kindex=1,numlev)
c	  print *,' tranr',(tranr(kindex),kindex=1,numlev)
c	  print *,'      ',eps,tsurf,prof.rcalc(n)
c	  stop
...
c        print *, 'read close status = ', rtpstat
c
        rtpstat = rtpclose(chan2)
c        print *, 'write close status = ', rtpstat, '  num.recs = ',i
c
        stop
        end
