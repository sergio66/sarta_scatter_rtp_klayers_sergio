...
C------------ ----------------- ----------------------------------------
C 24 Feb 1998 Scott Hannon      Created based on ourlay
C 15 Nov 2000 Scott Hannon      Move PLEV to common block "cbplev.f"
C 21 Nov 2000 Scott Hannon      Add check of MRTH2O to dry conversion
C 31 Jan 2001 Scott Hannon      Create version for RTP file I/O
C  9 Mar 2001 Scott Hannon      Change HBOUND from km to meters
C 23 Mar 2001 Scott Hannon      Get rid of altitude at 1100 mb and
C                                  replace with alt at bottom of
C                                  bottom-most used layer.  This is
C                                  calc'ed in intlev by new routine
C                                  ZBOT using PSURF and ZSURF info.
C                                  Minor changes to calls of RDINFO,
C                                  RDRTP, INTLEV, INTEG, & WRTWRTP.
C                                  Add check of PSURF & ZSURF.  Change
C                                  loops 1 to MYNLAY into LAYBOT to
C                                  MYNLAY.
C  9 May 2001 Scott Hannon      Change "IF" statement with label 20
C                                  from .GT. to .GE..
C 15 May 2001 Scott Hannon      Fix HBOUND()=ZPMAX altitude to correct
C                                  array position=(LAYBOT), not (1).
C 24 Jul 2001 Scott Hannon      Add CO2MLT
C 20 Nov 2001 Scott Hannon      Add NAMGRD to COMLEV
C  6 Dec 2001 Scott Hannon      ZSURF & ZPMAX calcs use scale height of 
C                                  SCALEH+LOG(P+1)/5 instead of SCALEH
C  8 Feb 2002 Scott Hannon      Add call to sub-routine SETXOP
C 29 Apr 2002 Scott Hannon      Bug fixes for case LZ=TRUE
C 05 Aug 2003 Scott Hannon      Change FIN & FOUT to CHAR*80 (was 70)
C 20 Feb 2007 Scott Hannon      Change all gases LDRY to LDRYF(MXGAS)
C                                  for user RTP gases and LDRYA for
C                                  AFGL gases and modify dry-to-wet air
C                                  code accordingly; remove MRTH2O.
C 06 Sep 2007 Scott Hannon      Add cloud particle size variables NPS
C                                  & INDPS and call to LAYPS.
...
C
       ...
       LOGICAL LSVP
       LOGICAL LTAIR
       LOGICAL  LDRYA          ! LDRY for AFGL gases
       LOGICAL  LDRYF( MXGAS)  ! LDRY for gases found in input RTP
       LOGICAL LSPLIN
       LOGICAL LPMAX
       LOGICAL LAFGL
C
...
C      -----------------------------------------------
     ...
     $   MASSW, TOFF, SCALEH, LDRYA, LSVP, LSPLIN, LPMAX)
C
       IF (LSPLIN) THEN
...
C      Find index of water (if any) in input RTP
       IWAT=-1
       DO I=1,NGASF
          IF (GLISTF(I) .EQ. 1) IWAT=I
       ENDDO
C
...
C         Calc Z at Pmax using kludged adjustment to scale height
          ZPMAX=-1E+3*(SCALEH+LOG(PMAX+1)/5)*LOG(PMAX/1013.25)
C
...
C               Adjust ZPMAX based on PSURF & ZSURF
                ZSUM=ZSURF +
     $             1E+3*(SCALEH+LOG(PSURF+1)/5)*LOG(PSURF/1013.25)
             ELSE
C               Adjust ZPMAX based on PIN(1) & ZIN(1)
                ZSUM=ZIN(1) +
     $             1E+3*(SCALEH+LOG(PIN(1)+1)/5)*LOG(PIN(1)/1013.25)
             ENDIF
             ZPMAX=ZPMAX + ZSUM
          ELSE
...
C            Zsurf is probably -999 or -9999 nodate flag; calc default
             ZSURF=-1E+3*(SCALEH+LOG(PSURF+1)/5)*LOG(PSURF/1013.25)
C
             IF (LZ) THEN
C               Find highest user level at or below ZSURF
 ...
 30             IF (PIN(J) .GT. PSURF .AND. J .LT. NIN) THEN
                ...
                ENDIF
                ZSUM=ZIN(J) +
     $             1E+3*(SCALEH+LOG(PIN(J)+1)/5)*LOG(PIN(J)/1013.25)
                ZSURF=ZSURF + ZSUM
             ENDIF
C
...
C      --------------------
C      Check index of water
C      --------------------
C      Note: if 1 <= IWAT <= NGASF, then water is from input RTP
       IF (IWAT .LT. 0) THEN
C         Check for AFGL water
          DO I=NGASF+1,NGASES
             IF (GASID(I) .EQ. 1) IWAT=I
          ENDDO
C
          IF (IWAT .LT. 1) THEN
             IWAT=0 ! want to skip "check index" on subsequent loops
C
C            Water is required if any gas units (user or AFGL) are "dry"
             IF (LDRYA) THEN
                IF (NGASES .GT. NGASF) THEN
                   WRITE(IOERR,1031)
 1031              FORMAT('Error! Can not process (ldrya=T) dry AFGL',
     $             'gases without water')
                ENDIF
                IF (NINX .GT. NIN) THEN
                   WRITE(IOERR,1033)
 1033              FORMAT('Error! Can not process (ldrya=T) dry AFGL',
     $             'profile extensions of user gases without water')
                ENDIF
             ENDIF
             DO I=1,NGASF
                IF (LDRYF(I)) THEN
                   WRITE(IOERR,1035) GLISTF(I)
 1035              FORMAT('Error! Can not process dry user gas ',
     $             ' ID=',I3,' without water')
                ENDIF
             ENDDO
          ENDIF
...
C      -----------------------------------------------------
C      Note: AFGL-only gases and the extended portion of user
C      profiles are "dry"
C
       IF (IWAT .GT. 0) THEN
C
C         Must adjust water first (if dry)
C         Adjust user profile levels if needed
          IF ((IWAT .LE. NGASF .AND. LDRYF(IWAT)) .OR.
     $        (IWAT .GT. NGASF .AND. LDRYA)) THEN
             DO I=1,NIN
                MRIN(I,IWAT)=1.0E+6*MRIN(I,IWAT)/(MRIN(I,IWAT)+1.0E+6)
             ENDDO
          ENDIF
C         Adjust AFGL extension levels if needed
          IF (NINX .GT. NIN .AND. LDRYA) THEN
             DO I=NIN+1,NINX
                MRIN(I,IWAT)=1.0E+6*MRIN(I,IWAT)/(MRIN(I,IWAT)+1.0E+6)
             ENDDO
          ENDIF
...
C            Do not adjust water again or clouds
             IF (J .NE. IWAT .AND. GASID(J) .LT. IDCLD1) THEN

C               Adjust AFGL gases and extension levels if needed
                IF (LDRYA) THEN
       
C                  AFGL gases; user profile levels only
                   IF (J .GT. NGASF) THEN
                      DO I=1,NIN
     ...
     $                   1.0E+6
                   ...
                   ENDIF
C
C                  All gases; AFGL extension levels only
                   DO I=NIN+1,NINX
                      MRIN(I,J)=MRIN(I,J)*(1.0E+6 - MRIN(I,IWAT))/1.0E+6
                   ENDDO
                ENDIF
C
C               Check user input gases
                IF (J .LE. NGASF .AND. LDRYF(J)) THEN
                   DO I=1,NIN
                      MRIN(I,J)=MRIN(I,J)*(1.0E+6 - MRIN(I,IWAT))/1.0E+6
                   ENDDO
                ENDIF
...
C
ccc Special case for cloud particle size: interpolate not integrate
       NPS=0
       DO J=1,NGASES
          IF (GASID(J) .GE. IDCLD1+IOFFPS) THEN
             NPS=NPS + 1
             INDPS(NPS)=J
          ENDIF
       ENDDO
       IF (NPS .GT. 0) THEN
          CALL LAYPS(NPS,INDPS,LAYBOT,NSUB,PSUB,TSUB,MRSUB,DZSUB,ALAY)
       ENDIF
ccc
C      -----------------------------------
