...
c       CHARACTER*70 FNAFGL         ! AFGL filename
       CHARACTER*132 FNAFGL         ! AFGL filename
C
C      Local variables
       INTEGER      I
       INTEGER   IGAS
...
C      --------
C
C      READ MINOR GAS CONCENTRATIONS 
C      Gas Den[ppmv] for minor gases in order of ID
C      from 8(NO), 9(SO2), etc through to 28(PH3) etc
C
  110  READ(IND,1020) KEY
       IF (KEY .NE. 'MINGAS ') GOTO 110 
C
C      -----------------------------
C      Read in minor gas mixng ratio     
  120  READ(IND,1000) WORDIN
       IF (WORDIN .EQ. COMMNT) GOTO 120
       IF (WORDIN .NE. COMMNT) BACKSPACE(IND)
       READ(IND,1020) KEY
       IF (KEY .EQ. 'DATEND ') GOTO 130 
C
       BACKSPACE(IND)     
       READ(IND,*) IGAS
C
