...
int rtpwrite1(char *fname,		/* RTP file name 	     IN  */
  ...
  *pchan = pci;
  ...
  for (i=0; i < NHFIELD; i++) {
    ...
    /* the header max fields sizes memis and mlevs
     * don't get put into the header as explicit values, so we
     * size to zero here so they will be dropped */
    if (strcmp(hsf[i].fname, "memis")==0 ||
  ...
  */
  ...
  for (i=0; i < npsf; i++) {
    ...
    if (strcmp((*pvf)[i].fname, "efreq" )==0 ||
	strcmp((*pvf)[i].fname, "emis"  )==0 ||
	strcmp((*pvf)[i].fname, "cemis" )==0 ||
	strcmp((*pvf)[i].fname, "cemis2")==0) 
      (*pvf)[i].order = head->memis;

    /* also set reflectance field orders to head.memis */
    if (strcmp((*pvf)[i].fname, "rho"  )==0 ||
	strcmp((*pvf)[i].fname, "crho" )==0 ||
	strcmp((*pvf)[i].fname, "crho2")==0)
      (*pvf)[i].order = head->memis;
    ...
    /* set the "gtotal" & "gxover" field orders to head.ngas */
    if (strcmp((*pvf)[i].fname, "gtotal")==0 || 
        strcmp((*pvf)[i].fname, "gxover")==0)
      (*pvf)[i].order = head->ngas;

    /* if we have observed radiance data, set the robs1 & calflag field 
       sizes to head.nchan, otherwise set it to zero */
    if (strcmp((*pvf)[i].fname, "robs1")==0 )
      (*pvf)[i].order = (head->pfields & IROBSVBIT) ? head->nchan : 0;
    if (strcmp((*pvf)[i].fname, "calflag")==0) 
      (*pvf)[i].order = (head->pfields & IROBSVBIT) ? head->nchan : 0;

    /* if we have calculated radiance data, set the rcalc field size to 
