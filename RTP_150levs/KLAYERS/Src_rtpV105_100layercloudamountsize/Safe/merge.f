...
C 31 Aug 2007 Scott Hannon      Add particle size extension
...
c       INTEGER IDAFGL(  MXIN)   ! AFGL gas IDs
       ...
       INTEGER  LISTG( MXGAS)   ! wanted gas IDs
...
C      ---------------------------------------------------------
       DO I=1,NGASES
          ...
          ENDDO
C
C         Set cloud particle size extension
          IF (GASID(I) .GE. IDCLD1+IOFFPS) THEN
             DO ILEV=NFIRST,NLAST MRIN(ILEV,I)=MRIN(NIN,I)
          ENDDO
       ENDDO
C
       RETURN
