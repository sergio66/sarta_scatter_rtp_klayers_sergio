...
C       NWANT, LISTG, MASSW, TOFF, SCALEH, LDRYA, LSVP, LSPLIN, LPMAX)
...
C    LOGICAL   LDRYA   T/F AFGL mix ratios dry air none
...
C
C    scaleh : scale height for Pres=1013.25*e^(Alt/scaleh).  This
...
C
C    ldrya : AFGL mixing ratios are dry air?; logical.  true/false or T/F
C
...
C 20 Feb 2007 Scott Hannon      Rename LDRY to LDRYA (now AFGL only)
C 22 Aug 2007 Scott Hannon      Added checks for cloud particle size;
C                                  resize IGJUNK from NGIDS+1 to MXGAS+1
...
C      =================================================================
     ...
     $    NWANT, LISTG, MASSW, TOFF, SCALEH, LDRYA, LSVP, LSPLIN, LPMAX)
C      =================================================================
...
C-----------------------------------------------------------------------
       ...
       INTEGER IDCMAX
       INTEGER IG
       INTEGER IGJUNK(MXGAS+1)  ! junk gas id work array
       INTEGER IARGC
       INTEGER IWAT
       INTEGER J
       INTEGER K
...
C      ------------
       ...
       MNAFGL=6                     ! AFGL model 6 = US Standard
       TOFF=0                       ! Temperature offset (usually 0)
       SCALEH=7.0                   ! scale height (km)
       LDRYA=.TRUE.                 ! AFGL mixing ratios for dry air?
...
C      Note: Set NWANT=-1 to use all gases
       NWANT=8                      ! AIRS "v5" RTA needs 8 gases:
       LISTG(1)=1                   !  1 = H2O
       LISTG(2)=2                   !  2 = CO2
       LISTG(3)=3                   !  3 = O3
       LISTG(4)=4                   !  4 = N2O
...
C            amount
             ...
             MASSW(NGIDS+IG)=1.0
C            particle size
             LISTG(NGIDS+MXCLD+IG)=IDCLD1 + IOFFPS + IG - 1
             MASSW(NGIDS+MXCLD+IG)=1.0
          ENDDO
C
       ELSE
...
C                  cloud amount profile
                   MASSW(IG)=1.0
                   I=NGIDS + ID - IDCLD1 + 1
                   IGJUNK(I)=IGJUNK(I) + 1
                ELSE
                   IF (ID .GE. IDCLD1+IOFFPS .AND. 
     $                 ID .LE. IDCMAX+IOFFPS) THEN
C                     cloud particle size profile
                      MASSW(IG)=1.0
                      I=NGIDS + ID - IDCLD1 - IOFFPS + MXCLD + 1
                      IGJUNK(I)=IGJUNK(I) + 1
                   ELSE
...
c          IWAT=0
c          DO I=1,NWANT
c             IF (LISTG(I) .EQ. 1) IWAT=I
c          ENDDO
c          IF (IWAT .EQ. 0) THEN
c             WRITE(IOERR,1050)
c 1050        FORMAT('ERROR!, LDRY=TRUE but LISTG omits water (ID=1)!')
c             STOP
c          ENDIF
c       ENDIF
cC
