...
C 15 Aug 2013 S.Machado      Try to revive 100 layer code
...
c       REAL CRHOS1(MXCHAN) ! chan solar reflectivity cloud1
c       REAL CRHOT1(MXCHAN) ! chan thermal reflectivity cloud1
c       REAL CRHOS2(MXCHAN) ! chan solar reflectivity cloud2
c       REAL CRHOT2(MXCHAN) ! chan thermal reflectivity cloud2

C
C      for CALRAD
       REAL COSANG(MAXLAY) ! local path angle cosine
...
C      Other variables for the sun
       ...
       REAL SCOSL(MAXLAY)  ! cosine of sun zenith angle in layer
       REAL SSECL(MAXLAY)  ! secant of sun zenith angle in layer
...
C     TWO CLOUD INFO
c       INTEGER IDCLD1 ! cloud1 ID number
c       INTEGER IDCLD2 ! cloud2 ID number
c       INTEGER IDCLD3 ! aerosol ID number

C      Basic cloud info
       INTEGER ICLD1          ! index of cloud1 in HEAD.glist
       INTEGER ICLD2          ! index of cloud2 in HEAD.glist
       INTEGER ICLD3          ! index of aerosol in HEAD.glist
       ...
       REAL CNGWA1(MAXLAY)    ! cloud1 non-gases water
       REAL CNGWA2(MAXLAY)    ! cloud1 non-gases water
       ...
       REAL TLAC1             ! cloud1 frac layer (above cloud) mean temp
       REAL TLAC2             ! cloud2 frac layer (above cloud) mean temp
...
C      for CCPREP cloud1
       INTEGER LBOTC1         ! layer containing cloud bottom
       INTEGER LTOPC1         ! layer containing cloud top
       REAL  CLRT1            ! frac of layer at top of cloud clear
       REAL TTOPC1            ! temperature at cloud top
       REAL G_ASY1(MXCHAN)    ! "g" asymmetry
       REAL NEXTB1(MXCHAN)    ! nadir extinction optical depth
       REAL NSCAB1(MXCHAN)    ! nadir scattering optical depth
       REAL xG_ASY1(MAXLAY,MXCHAN)    ! "g" asymmetry  with lay
       REAL xNEXTB1(MAXLAY,MXCHAN)    ! nadir extinction with lay
       REAL xNSCAB1(MAXLAY,MXCHAN)    ! nadir scattering with lay
C
C      for CCPREP cloud2
       INTEGER LBOTC2         ! layer containing cloud bottom
       INTEGER LTOPC2         ! layer containing cloud top
       REAL  CLRT2            ! frac of layer at top of cloud clear
       REAL TTOPC2            ! temperature at cloud top
       REAL G_ASY2(MXCHAN)    ! "g" asymmetry
       REAL NEXTB2(MXCHAN)    ! nadir extinction optical depth
       REAL NSCAB2(MXCHAN)    ! nadir scattering optical depth
       REAL xG_ASY2(MAXLAY,MXCHAN)    ! "g" asymmetry  with lay
       REAL xNEXTB2(MAXLAY,MXCHAN)    ! nadir extinction with lay
       REAL xNSCAB2(MAXLAY,MXCHAN)    ! nadir scattering with lay
C
C      for combined complex cloud
       INTEGER LBOT12         ! layer containing cloud1+2 bottom
       INTEGER LTOP12         ! layer containing cloud1+2 top

C
C      Cloud calc logical switches
       LOGICAL LCLR           ! calc clear?
       LOGICAL LB1            ! calc black cloud1?
       LOGICAL LC1            ! calc complex cloud1?
       LOGICAL LB2            ! calc black cloud2?
       LOGICAL LC2            ! calc complex cloud2?
       LOGICAL LB1C2          ! calc black cloud1 + complex cloud2?
       LOGICAL LC1B2          ! calc complex cloud1 + black cloud2?
       LOGICAL LC1C2          ! calc complex cloud1 + complex cloud2?
       LOGICAL LC1L(MAXLAY)   ! layer complex cloud1?
       LOGICAL LC2L(MAXLAY)   ! layer complex cloud2?
       LOGICAL LC12L(MAXLAY)  ! layer complex cloud1 + complex cloud2?
C
C      used locally only
...
C      Find cloud IDs if present in HEAD.glist
       ICLD1=-1
       ICLD2=-1
       ICLD3=-1
       DO I=1,HEAD.ngas
          IF (HEAD.glist(I) .EQ. IDCLD1) ICLD1 = I
          IF (HEAD.glist(I) .EQ. IDCLD2) ICLD2 = I
          IF (HEAD.glist(I) .EQ. IDCLD3) ICLD3 = I
       ENDDO
       IF (ICLD3 .GT. 0) THEN
         write(*,*) 'sarta_pclsam : can only set gas201,gas202'
         STOP
       END IF  
...
C      ---------------------------------------------------
       CALL SETEMS( NCHAN, NEMIS, FREQ, XEMIS, FEMIS,
...
C      Get basic cloud info from input RTP
       CALL GETCLD_PROFILE( IPROF, ICLD1, ICLD2, HEAD, PROF,
...
ccc uncomment this block for testing
c       print *, 'iprof,cfrac1,ctype1,lblac1',IPROF,CFRAC1,CTYPE1,LBLAC1
c       print *, 'iprof,cfrac2,ctype2,lblac2',IPROF,CFRAC2,CTYPE2,LBLAC2
c       print *, 'iprof,fclear,cfra12,cfra1x,cfra2x=',
c     $    IPROF,FCLEAR,CFRA12,CFRA1X,CFRA2X
c       print *,' '
ccc

C      Prepare cloud1 for RTA calc
       IF (CFRAC1 .GT. 0.0) THEN
          IF (LBLAC1) THEN
             CALL BKPREP(IPROF, 1, CTYPE1, CFRAC1, CPRTO1, CEMIS1,
     $          LBOT, PSURF, PLEV, PLAY, TEMP, LTOPC1, CRHO1, TTOPC1,
     $          TLAC1, CLRT1)
          ELSE
...
C            Prepare selected lookup table for given cpsize
             Call CCPREP_PROFILE( NCHAN, LBOT, INDMI1, MIENPS, 
     $          CNGWA1, CPSIZ1, MIEPS, MIEABS, MIEEXT, MIEASY,
     $          LBOTC1, LTOPC1, 
     $          G_ASY1, NEXTB1, NSCAB1, 
     $          xG_ASY1, xNEXTB1, xNSCAB1, 
     $          TEMP, CTYPE1)
          ENDIF
       ENDIF

C      Prepare cloud2 for RTA calc
       IF (CFRAC2 .GT. 0.0) THEN
          IF (LBLAC2) THEN
             CALL BKPREP(IPROF, 2, CTYPE2, CFRAC2, CPRTO2, CEMIS2,
     $          LBOT, PSURF, PLEV, PLAY, TEMP, LTOPC2, CRHO2, TTOPC2,
     $          TLAC2, CLRT2)
          ELSE
C            Determine which lookup table to use
             CALL GETMIE(CTYPE2,MIETYP,INDMI2,IERR2)
C            Prepare selected lookup table for given cpsize
             Call CCPREP_PROFILE( NCHAN, LBOT, INDMI2, MIENPS,
     $          CNGWA2, CPSIZ2, MIEPS, MIEABS, MIEEXT, MIEASY,
     $          LBOTC2, LTOPC2, 
     $          G_ASY2, NEXTB2, NSCAB2, 
     $          xG_ASY2, xNEXTB2, xNSCAB2, 
     $          TEMP, CTYPE2)
          ENDIF
       ENDIF

ccc uncomment for testing
c      if (IPROF .EQ. 1) then
c        print *, 'ctype1,ltopc1,tbotc1,cpsize1=',
c     $            CTYPE1,LTOPC1,LBOTC1,CPSIZ1
c        print *, 'ctype2,ltopc2,tbotc2,cpsize2=',
c     $            CTYPE2,LTOPC2,LBOTC2,CPSIZ2
cc        do I=1,LBOT
cc          print *,'layer, cngwa1, cngwa2=',I,CNGWA1(I),CNGWA2(I)
cc        enddo
c       print *,' '
c      endif
ccc

C      Set logical switches for the 8 possible cloud calcs
       LCLR =.FALSE. ! clear
       LB1  =.FALSE. ! black1
       LC1  =.FALSE. ! complex1
       LB2  =.FALSE. ! black2
       LC2  =.FALSE. ! complex2
       LB1C2=.FALSE. ! black1 + complex2
       LC1B2=.FALSE. ! complex1 + black2
       LC1C2=.FALSE. ! complex1 + complex2
       IF (FCLEAR .GT. 0.0) LCLR=.TRUE.
       IF (CFRAC1 .GT. 0.0) THEN
          IF (LBLAC1) THEN
             IF (CFRA1X .GT. 0) LB1=.TRUE.
             IF (CFRA12 .GT. 0) LB1C2=.TRUE.
          ELSE
             IF (CFRA1X .GT. 0) LC1=.TRUE.
             IF (CFRA12 .GT. 0) LC1C2=.TRUE.
          ENDIF
       ENDIF
       IF (CFRAC2 .GT. 0.0) THEN
          IF (LBLAC2) THEN
             IF (CFRA2X .GT. 0) LB2=.TRUE.
             IF (CFRA12 .GT. 0) LC1B2=.TRUE.
          ELSE
             IF (CFRA2X .GT. 0) LC2=.TRUE.
          ENDIF
       ENDIF
C

ccc uncomment for testing
c       print *, 'IPROF,LCLR,LB1,LC1,LB2,LC2,LB1C2,LC1B2,LC1C2=',
c     $           IPROF,LCLR,LB1,LC1,LB2,LC2,LB1C2,LC1B2,LC1C2
c       print *, 'CFRA1X,CFRA2X,CFRA12  = ',CFRA1X,CFRA2X,CFRA12
ccc

C      Determine top and bottom layers for combined cloud
       IF (LC1C2) THEN
          LTOP12=MIN0(LTOPC1,LTOPC2)
          LBOT12=MAX0(LBOTC1,LBOTC2)
       ENDIF
C
C      Initialize the possible radiance contributes
       RAD0  =0.0 ! clear
       RADC1 =0.0 ! cloud1 only
       RADC2 =0.0 ! cloud2 only
       RADC12=0.0 ! cloud1 + cloud2
C
C      Solar irradiance term
...
C      Determine which layers require complex cloud calcs
       DO L=1,LBOT
          LC1L(L) =.FALSE.
          LC2L(L) =.FALSE.
          LC12L(L)=.FALSE.
          IF (CNGWA1(L) .GT. CNGMIN) THEN
             IF (LC1)   LC1L(L) =.TRUE.
             IF (LC1B2) LC1L(L) =.TRUE.
             IF (LC1C2) LC12L(L)=.TRUE.
          ENDIF
          IF (CNGWA2(L) .GT. CNGMIN) THEN
             IF (LC2)   LC2L(L) =.TRUE.
             IF (LB1C2) LC2L(L) =.TRUE.
             IF (LC1C2) LC12L(L)=.TRUE.
          ENDIF
       ENDDO
C
...
ccc uncomment this block for testing
c          print *,'I,EMIS(I),TSURF,RSURF = ',I,EMIS(I),TSURF,RSURFE
ccc

C         Calculate clear radiance
          IF (LCLR) THEN
             CALL CALRAD0( DOSUN, I, LBOT, RPLNCK, RSURFE, SECANG,
     $       TRANL, TRANZ, SUNFAC, HSUN, TRANS, RHOSUN(I),
     $       RHOTHR(I), COEFF, RAD0 )
          ELSE
             RAD0=0.0
          ENDIF

C         Calculate black cloud1
          IF (LB1 .OR. LB1C2) THEN

C            Store original values
             VSTORE(1)=TRANL(LTOPC1)
             VSTORE(2)=TRANZ(I)
             VSTORE(3)=TRANS(I)
             VSTORE(4)=RPLNCK(LTOPC1)
C
C            Update values for black cloud1
             RJUNK1=-TAU(LTOPC1,I)*CLRT1
             TRANL(LTOPC1)=QIKEXP( RJUNK1 )
             TRANZ(I)=QIKEXP( RJUNK1 - TAUZ(LTOPC1-1,I) )
             TRANS(I)=QIKEXP( CLRT1*(TAUZSN(LTOPC1-1,I)-
     $          TAUZSN(LTOPC1,I)) - TAUZSN(LTOPC1-1,I) )
             RSURFC=CEMIS1*C1V3/( EXP( C2V/TTOPC1 ) - 1.0 )
             RPLNCK(LTOPC1)=C1V3/( EXP( C2V/TLAC1 ) - 1.0 )

C            Calc black1
             IF (LB1) THEN
                CALL CALRAD0( DOSUN, I, LTOPC1, RPLNCK, RSURFC, SECANG,
     $          TRANL, TRANZ, SUNFAC, HSUN, TRANS, CRHO1,
     $          CRHO1, COEFF, RADC1 )
             ENDIF

C            Calc black1 + complex2
             IF (LB1C2) THEN
                CALL CALRAD1( DOSUN, LC2L, I, LTOPC1, RPLNCK, RSURFC,
     $             COSANG, SECANG, SCOSL, SSECL, TAU, TRANL, TRANZ,
     $             SUNFAC, HSUN, TRANS, CRHO1, CRHO1, COEFF, CNGWA2,
     $             NEXTB2, NSCAB2, G_ASY2, xNEXTB2, xNSCAB2, xG_ASY2,
     $             LTOPC2, LBOTC2, RADC12 )
             ENDIF
C
C            Restore original values
             TRANL(LTOPC1) =VSTORE(1)
             TRANZ(I)      =VSTORE(2)
             TRANS(I)      =VSTORE(3)
             RPLNCK(LTOPC1)=VSTORE(4)
          ENDIF

C         Calc complex cloud1
          IF (LC1) THEN
             CALL CALRAD1( DOSUN, LC1L, I, LBOT, RPLNCK, RSURFE,
     $          COSANG, SECANG, SCOSL, SSECL, TAU, TRANL, TRANZ,
     $          SUNFAC, HSUN, TRANS, RHOSUN(I), RHOTHR(I), COEFF,
     $          CNGWA1, 
     $          NEXTB1, NSCAB1, G_ASY1, xNEXTB1, xNSCAB1, xG_ASY1,
     $          LTOPC1, LBOTC1, RADC1 )
          ENDIF
C
C         Calc black cloud2
          IF (LB2 .OR. LC1B2) THEN
C
C            Store original values
             VSTORE(1)=TRANL(LTOPC2)
             VSTORE(2)=TRANZ(I)
             VSTORE(3)=TRANS(I)
             VSTORE(4)=RPLNCK(LTOPC2)

C            Update values for black cloud2
             RJUNK1=-TAU(LTOPC2,I)*CLRT2
             TRANL(LTOPC2)=QIKEXP( RJUNK1 )
             TRANZ(I)=QIKEXP( RJUNK1 - TAUZ(LTOPC2-1,I) )
             TRANS(I)=QIKEXP( CLRT2*(TAUZSN(LTOPC2-1,I)-
     $          TAUZSN(LTOPC2,I)) - TAUZSN(LTOPC2-1,I) )
             RSURFC=CEMIS2*C1V3/( EXP( C2V/TTOPC2 ) - 1.0 )
             RPLNCK(LTOPC2)=C1V3/( EXP( C2V/TLAC2 ) - 1.0 )

C            Calc black2
             IF (LB2) THEN
                CALL CALRAD0( DOSUN, I, LTOPC2, RPLNCK, RSURFC, SECANG,
     $          TRANL, TRANZ, SUNFAC, HSUN, TRANS, CRHO2,
     $          CRHO2, COEFF, RADC2 )
             ENDIF

C            Calc complex1+black2
             IF (LC1B2) THEN
                CALL CALRAD1( DOSUN, LC1L, I, LTOPC2, RPLNCK, RSURFC,
     $             COSANG, SECANG, SCOSL, SSECL, TAU, TRANL, TRANZ,
     $             SUNFAC, HSUN, TRANS, CRHO2, CRHO2, COEFF, CNGWA1,
     $             NEXTB1, NSCAB1, G_ASY1, xNEXTB1, xNSCAB1, xG_ASY1, 
     $             LTOPC1, LBOTC1, RADC12 )
             ENDIF
C
C            Restore original values
             TRANL(LTOPC2) =VSTORE(1)
             TRANZ(I)      =VSTORE(2)
             TRANS(I)      =VSTORE(3)
             RPLNCK(LTOPC2)=VSTORE(4)
          ENDIF

C         Calc complex cloud2
          IF (LC2) THEN
             CALL CALRAD1( DOSUN, LC2L, I, LBOT, RPLNCK, RSURFE,
     $          COSANG, SECANG, SCOSL, SSECL, TAU, TRANL, TRANZ,
     $          SUNFAC, HSUN, TRANS, RHOSUN(I), RHOTHR(I), COEFF,
     $          CNGWA2, 
     $          NEXTB2, NSCAB2, G_ASY2, xNEXTB2, xNSCAB2, xG_ASY2,
     $          LTOPC2, LBOTC2, RADC2 )
          ENDIF
C
C         Calc complex cloud1 + complex cloud2
          IF (LC1C2) THEN
             CALL CALRAD2( DOSUN, LC12L, I, LBOT, RPLNCK, RSURFE,
     $          COSANG, SECANG, SCOSL, SSECL, TAU, TRANL, TRANZ,
     $          SUNFAC, HSUN, TRANS, RHOSUN(I), RHOTHR(I), COEFF,
     $          CNGWA1, NEXTB1, NSCAB1, G_ASY1,
     $          CNGWA2, NEXTB2, NSCAB2, G_ASY2, 
     $                  xNEXTB1, xNSCAB1, xG_ASY1,
     $                  xNEXTB2, xNSCAB2, xG_ASY2, 
     $          LTOP12, LBOT12, RADC12)
          ENDIF
C
...
c          IF (I .EQ. 1291) THEN
c            print *,IPROF,FCLEAR,CFRA1X,CFRA2X,CFRA12,'>>',
c     $              RAD0,RADC1,RADC2,RADC12,RAD(I)
c          ENDIF
