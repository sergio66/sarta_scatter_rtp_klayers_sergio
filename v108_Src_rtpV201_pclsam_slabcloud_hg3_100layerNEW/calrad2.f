...
C       CNGWA2, NEXTB2, NSCAB2, G_ASY2, 
c               xNEXTB1, xNSCAB1, xG_ASY1,
c               xNEXTB2, xNSCAB2, xG_ASY2, 
C       LTOPC ,LBOTC, RAD12 )
...
C    REAL matr x NEXTB1  cloud1 extinction           per g/m^2
C    REAL matr  xNSCAB1  cloud1 scattering           per g/m^2
C    REAL matr  xG_ASY1  cloud1 asymmetry parameter  none
...
C    REAL matr x NEXTB2  cloud1 extinction           per g/m^2
C    REAL matr  xNSCAB2  cloud1 scattering           per g/m^2
C    REAL matr  xG_ASY2  cloud1 asymmetry parameter  none
...
c      for combining scattering properties, see
c JOURNAL OF GEOPHYSICAL RESEARCH, VOL. 97, NO. D7, PAGES 7603-7612, MAY 20, 1992
c Delta-Eddington Approximation for Solar Radiation
c in the NCAR Community Climate Model
c BRUCE P. BRIEGLEB
      REAL NG1,NG2,NOMEGA1,NOMEGA2,EPS
      REAL NOMEGA
...
C
             NEXTO1 = xNEXTB1(L,I)*CNGWA1(L)
             NSCAO1 = xNSCAB1(L,I)*CNGWA1(L)
c             NOMEGA1 = NSCAO1/NEXTO1
             NOMEGA1 = xNSCAB1(L,I)/xNEXTB1(L,I)
             NG1    = xG_ASY1(L,I)

             NEXTO2 = xNEXTB2(L,I)*CNGWA2(L)
             NSCAO2 = xNSCAB2(L,I)*CNGWA2(L)
c             NOMEGA2 = NSCAO2/NEXTO2
             NOMEGA1 = xNSCAB2(L,I)/xNEXTB2(L,I)
             NG2    = xG_ASY1(L,I)

c orig >>>>
c             NEXTOD = NEXTO1 + NEXTO2
c             NSCAOD = NSCAO1 + NSCAO2
c             GL(L)=( G_ASY1(I)*NSCAO1 + G_ASY2(I)*NSCAO2 )/NSCAOD
c orig >>>>

c new, using combination formulas >>>>
             NEXTOD = NEXTO1 + NEXTO2

             NOMEGA = (NOMEGA1*NEXTO1 + NOMEGA2*NEXTO2 + EPS)/
     $                (NEXTOD + EPS)
             NSCAOD = NOMEGA*NEXTOD

             GL(L) = (NOMEGA1*NEXTO1*NG1 + NOMEGA2*NEXTO2*NG2 + EPS)/
     $               (NEXTOD*NOMEGA + EPS)
c new, using combination formulas >>>>
...
c       print *,'badoom',RHOTHR,PI,RDOWN,F,TRANZ(I),TRANCU
C

C      --------------
C      Total radiance
C      --------------
c       print *,'hohum C',RADUP,RSUN,RTHERM
